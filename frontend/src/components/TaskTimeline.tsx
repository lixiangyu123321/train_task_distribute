        ...
        return (
          <div key={step} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
            <div style={{ textAlign: 'center', zIndex: 1 }}>
              ...
              <div style={{ marginTop: 4, color: col, transition: 'color 0.3s' }}>{isFailed && step === 'RUNNING' ? 'FAIL' : labels[step]}</div>
            </div>
            {i < steps.length - 1 && (
              <div style={{ flex: 1, height: 3, margin: '0 4px', marginTop: -14, borderRadius: 2, background: done && i < idx ? `repeating-linear-gradient(90deg, ${col} 0px, ${col} 6px, transparent 6px, transparent 8px)` : '#2a3548', transition: 'background 0.3s' }} />
            )}
          </div>
        );
      })}
